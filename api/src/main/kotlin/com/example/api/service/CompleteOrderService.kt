...
class CompleteOrderService(
    ...
    fun completeOrder(eslOrderNumber: Int) {
        ...
        val orders = orderRepository.findAllByDoneFalseAndEslOrderNumberLessThanEqual(eslOrderNumber)
        // orders에 eslOrderNumber이하의 데이터들은 자기 자신의 다음 데이터로 덮어씌우기, 만약 다음 값이 없으면 빈 값
        ...
        val eslOrders = eslOrderRepository.findAllByOrderNumberLessThanEqualOrderByOrderNumberDesc(eslOrderNumber.toLong())

        for (i in eslOrderNumber downTo 1) {
            val eslOrdersIndex = eslOrderNumber - i
            val imagesIndex = eslOrderNumber - i

            val image = if (imagesIndex >= images.size) {
